...
var fetchLast = function (host, file, cf, query) {
  return function (cb) {
    fetch(host, file, cf, query)(function (err, data) {
      if (err) {
        cb(err);
      } else {
        cb(null, _.last(data));
...
var normalizeLoad = function (cols, host) {
  return function (data, callback) {
    ...
    _.each(data, function (e, host) {
      var numberOfCpus = countCpu(host);
      _(cols).each(function (i) { e[i] = e[i] / numberOfCpus; });
    });
    
    callback(null, host ? data[host] : data);
  };
};

var countCpu = function (host) {
  var dir = [config['data-directory'], host].join('/');
  var str = "cpu-";
  
  return _.filter(fs.readdirSync(dir), function (e) {
...
var timer = function (time, period) {
  var now = time || config['last-timestamp'] || Math.round(new Date().getTime() / 1000);
