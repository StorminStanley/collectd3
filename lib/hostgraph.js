...
module.exports = function (req, res, next) {
  ...
  var query = {
    ...
    // Here is an issue. Our last RRA have 2635 pdp per row (or 1 value per 7 hours and 20 minutes).
    // So, you need about 1200 rows to represent whole year and it will be bad idea to send them
    // all to client. Only option i see is that we need to reduce them server-side, but it means
    // we need to change our fetch logic to handle this case.
    //
    // year: {
    //   from: now - 31536000,
    //   to: now,
  ...
  return function (cb) {
    rrdhelpers.fetch(host, "load/load.rrd", "MAX", query)(function (err, data) {
      cb(err, _.map(data, function (e) {
        // TODO: DZ Why returning null?
        return [e._time, e.longterm, null, e.longterm / rrdhelpers.countCpu(host)];
      }));
    });
  };
};
...
var hostGraphMemory = function (host, query) {
  return function (cb) {
    async.parallel({
      ...
      var results = _(data.used).filter(function (e, i) {
        return data.used[i] && data.free[i] && data.buffered[i] && data.cached[i];
      }).map(function (e, i) {
        var total = data.used[i].value + data.free[i].value +
                data.buffered[i].value + data.cached[i].value;
        return [
          data.used[i]._time, // timestamp
          data.used[i].value / total * 100, // percentage used
          null,
          data.used[i].value,
          data.free[i].value
        ];
      ...
      if (err) {
        ...
        cb(null, _.map(data[type.disks[0]], function (e, i) {
          return [e._time].concat(_(type.disks).map(function (e) {
            return data[e][i] ? data[e][i].read + data[e][i].write : 0;
          }).reduce(function (a, b) {
            return a + b;
          }));
        }));
  ...
  return function (cb) {
    async.parallel({
      ...
      cb(err, _.map(data.traffic, function (e, i) {
        return [e._time, e.rx + e.tx, data.errors[i] ? data.errors[i].rx + data.errors[i].tx : 0];
      }));
    });
  };
};