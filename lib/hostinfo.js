 ...
 */
var hostInfoStorageIO = function (host, time) {
    ...
    if (list) {
      async.parallel({
        average: _.partial(async.parallel, _.map(list.disks, function (e) {
          return rrdhelpers.fetchLast(host, 'disk-' + e + '/disk_octets.rrd', "AVERAGE", rrdhelpers.timer(time));
        })),
        max: _.partial(async.parallel, _.map(list.disks, function (e) {
          return rrdhelpers.fetchLast(host, 'disk-' + e + '/disk_octets.rrd', "MAX", rrdhelpers.timer(time));
        }))
      }, function (err, data) {
        if (err) {
          cb(err);
 ...
 */
var hostInfoNetworkIO = function (host, time) {
  return function (cb) {
    async.parallel({
      average: rrdhelpers.fetchLast(host, "interface/if_octets-vlan11.rrd", "AVERAGE", rrdhelpers.timer(time)),
      max: rrdhelpers.fetchLast(host, "interface/if_octets-vlan11.rrd", "MAX", rrdhelpers.timer(time)),
      errors: rrdhelpers.fetchLast(host, "interface/if_errors-vlan11.rrd", "MAX", rrdhelpers.timer(time))
    }, function (err, data) {
      if (err) {
        cb(err);
      } else {
